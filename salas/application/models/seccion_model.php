...
class Seccion_model extends CI_Model {//@
	...
	function grid($p,$o){
		...
		//PAGINADOR
		$queryCount="SELECT COUNT(*) AS count FROM seccion as a ".$where;//@
		...
		FROM seccion as a,asignatura as asi,profesor as p ".$where." 
		and
		 asi.id=a.asignatura_id and p.id=a.profesor_id  ORDER BY $p->sidx $p->sord LIMIT $p->limit offset $start "; 
        ...
        if ($result->num_rows() > 0)
		{
			...
			foreach($result->result() as $rs){//@
				...
				$response->rows[$i]['cell']=array($rs->id,$rs->nombre,$rs->asignatura_id.$asignatura,$rs->profesor_id.$profesor);  
				$i++;
			}
	...
	function insertar($o){
		...
		$result=$this->db->insert("seccion",$query);//@
